    ...
    core_nodes = [
        ...
        Node(
            ...
            parameters=[{
                ...
                'map_name': map_name,
                # 2D SLAM doesn't need the 3D .ply, and saving it re-serializes
                # the whole accumulated cloud every cycle — disable during mapping.
                'map_save': 'false',
                # Cap accumulation: the node sorts the entire point set every
                # LiDAR frame, so the 1,000,000 default saturates a CPU core on a
                # lab-sized space and starves /scan. 60k keeps /scan alive.
                'max_points': 60000,
            }],
        ),
        ...
        Node(
            ...
            remappings=[
                # Consume the first node's capped single-source cloud
                # (/pointcloud/aggregated, ~60k pts) directly. The heavy
                # pointcloud_aggregator that sums 200 clouds into /pointcloud/
                # filtered saturates CPU and starves /scan, so bypass it for SLAM.
        ...
        # (No TTS node: the voice agent uses Nova Sonic speech-to-speech and
        # speaks host-side, so nothing publishes to /tts. Matches robot.launch.py.)
    ]
