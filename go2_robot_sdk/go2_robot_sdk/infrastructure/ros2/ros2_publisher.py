...
from go2_interfaces.msg import Go2State, IMU, LowState, BmsState, MotorState
...
class ROS2Publisher(IRobotDataPublisher):
    ...
    def publish_low_state(self, robot_data: RobotData) -> None:
        """Publish full LowState (battery, foot forces, motor temps, IMU)."""
        raw = robot_data.low_state_raw
        if not raw:
            return
        try:
            robot_idx = int(robot_data.robot_id)
            ...
            # IMU
            imu = raw.get("imu_state") or {}
            if imu:
                msg.imu_state.quaternion = list(map(float, imu.get("quaternion", [0.0] * 4)))
                msg.imu_state.gyroscope = list(map(float, imu.get("gyroscope", [0.0] * 3)))
                msg.imu_state.accelerometer = list(map(float, imu.get("accelerometer", [0.0] * 3)))
                msg.imu_state.rpy = list(map(float, imu.get("rpy", [0.0] * 3)))
                msg.imu_state.temperature = int(imu.get("temperature", 0))

            # Motors (LowState carries 20 slots; the robot only fills the first 12)
            motors = raw.get("motor_state") or []
            for i, slot in enumerate(msg.motor_state):
                if i >= len(motors):
                    break
                m = motors[i]
                slot.q = float(m.get("q", 0.0))
                slot.dq = float(m.get("dq", 0.0))
                slot.ddq = float(m.get("ddq", 0.0))
                slot.tau_est = float(m.get("tau_est", 0.0))
                slot.temperature = int(m.get("temperature", 0))
                slot.lost = int(m.get("lost", 0))

            # Battery
            bms = raw.get("bms_state") or {}
            if bms:
                msg.bms_state.version_high = int(bms.get("version_high", 0))
                msg.bms_state.version_low = int(bms.get("version_low", 0))
                msg.bms_state.status = int(bms.get("status", 0))
                msg.bms_state.soc = int(bms.get("soc", 0))
                msg.bms_state.current = int(bms.get("current", 0))
                msg.bms_state.cycle = int(bms.get("cycle", 0))
                msg.bms_state.bq_ntc = list(bms.get("bq_ntc", [0, 0]))
                msg.bms_state.mcu_ntc = list(bms.get("mcu_ntc", [0, 0]))

            # Foot forces and miscellaneous telemetry
            if "foot_force" in raw:
                msg.foot_force = list(raw["foot_force"])
            if "temperature_ntc1" in raw:
                msg.temperature_ntc1 = int(raw["temperature_ntc1"])
            if "power_v" in raw:
                msg.power_v = float(raw["power_v"])

            self.publishers['low_state'][robot_idx].publish(msg)
        except Exception as e:
            logger.error(f"Error publishing low state: {e}")
    ...
    def publish_camera_data(self, robot_data: RobotData) -> None:
        ...
        try:
            ...
            frame_id = ('front_camera' if self.config.conn_mode == 'single'
                        else f'robot{robot_data.robot_id}/front_camera')
            ros_image.header.frame_id = frame_id

            # The image is the important payload — always publish it. Camera
            # info (intrinsics) is keyed by frame height and only exists for
            # resolutions we have a calibration YAML for. Remote/TURN streams
            # can negotiate other resolutions (e.g. 360p), so a missing calib
            # must NOT drop the video feed — publish what we have and skip the
            # CameraInfo if it isn't available for this resolution.
            self.publishers['camera'][robot_idx].publish(ros_image)

            camera_info = self.camera_info.get(camera.height)
            if camera_info is not None:
                ...
                logger.warning(
                    f"No camera calibration for height {camera.height}; "
                    f"publishing image without CameraInfo"
                )
