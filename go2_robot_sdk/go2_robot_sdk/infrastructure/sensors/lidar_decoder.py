...
# SHA-256 of the libvoxel.wasm shipped in go2_robot_sdk/external_lib/, verified
# before the module is instantiated.
#
# This binary is a 9,520-byte WebAssembly blob with no accompanying source, no
# build recipe and no determinable license (see THIRD-PARTY-LICENSES), and it is
# EXECUTED at runtime by wasmtime. The pin does not fix the provenance problem —
# only upstream or a rebuild from source can — but it does mean the bytes that
# run are the bytes that were reviewed, so a swapped or corrupted file fails
# closed here instead of executing.
#
# If you legitimately replace the file, recompute and update this constant:
#   shasum -a 256 go2_robot_sdk/external_lib/libvoxel.wasm
LIBVOXEL_SHA256 = "1ba27f065199954dc562b233aa38ea775ac687118cce77b5c71bc14e17e05542"


def _verified_wasm_bytes(path: str) -> bytes:
    """Read the decoder blob and refuse to hand it back if the digest differs."""
    with open(path, "rb") as f:
        data = f.read()

    digest = hashlib.sha256(data).hexdigest()
    if digest != LIBVOXEL_SHA256:
        raise RuntimeError(
            f"libvoxel.wasm integrity check FAILED for {path}: expected "
            f"{LIBVOXEL_SHA256}, got {digest}. Refusing to execute it. If you "
            "replaced the file on purpose, update LIBVOXEL_SHA256 in "
            "lidar_decoder.py; otherwise restore the file from the repository."
        )
    return data
...
class LidarDecoder:
    ...
    def __init__(self) -> None:
        ...
        # from_binary, not from_file: the digest has to be checked against the
        # exact bytes that get compiled, and from_file would read them again.
        self.module = Module(
            self.store.engine, _verified_wasm_bytes(libvoxel_path)
        )
