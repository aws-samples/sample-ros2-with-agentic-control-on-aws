...
class WebRTCAdapter(IRobotDataReceiver, IRobotController):
    ...
    async def connect(self, robot_id: str) -> None:
        ...
        try:
            ...
            aes_128_key = (self.config.aes_128_key_list or [""])[robot_idx]
    ...
    async def request_keyframe(self, robot_id: str) -> bool:
        """Ask the robot for an H.264 keyframe (RTCP PLI) on the given connection."""
        conn = self.connections.get(robot_id)
        if conn is None:
            return False
        return await conn.request_keyframe()

    def is_connected(self, robot_id: str) -> bool:
        """True only if the robot's WebRTC data channel is open and usable.

        The robot (especially over the remote/TURN path) can tear down the peer
        connection while the Go2Connection object lingers, leaving a data_channel
        whose readyState is no longer 'open'. Sends into that state fail silently,
        so the control loop uses this to decide when to reconnect.
        """
        conn = self.connections.get(robot_id)
        if conn is None:
            return False
        channel = getattr(conn, 'data_channel', None)
